...
const defaultDeck =
  "AAEBAaIHCLICrwSSlwPBrgOCsQPjtAObtgPLwAMLtAHtAs0DiAePlwP1pwO5rgP+rgOqrwPOrwPDtgMA";
...
function getDeck(hearthStoneDeckId) {
  ...
  fetch(url, options)
    ...
    .catch((err) => {
      $("#error").append(
        '<span id="errorMessage"> Deck not found try again. </span>'
      );
    });
}
...
function showSummary(manaCardCount) {
  ...
  for (const [key, value] of Object.entries(manaCardCount)) {
    $("table").append(`<tr><td> ${value.mana} </td><td>${value.count}</td>`);
  }
}
